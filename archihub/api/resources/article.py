...
comments. The frontend sanitises them with DOMPurify before rendering.
...
def may_edit(user: str, resource: dict, is_admin: bool) -> bool:
    ...
    * Nobody may edit what they cannot read.
    * If the content type declares ``editRoles``, holding one of them is
      required and sufficient.
    * If it declares none, the rule of the ordinary resource-update route
      applies: the resource's creator, or a ``super_editor``.

    To let people edit articles on resources they did not create, declare
    ``editRoles`` on the content type or grant ``super_editor``.
    """
    if is_admin:
        return True
...
def update_article_body(resource_id: str, body: dict, user: str) -> tuple[dict, int]:
    ...
    ONLY ``articleBody`` IS WRITTEN. Status, access rights, content type and
    placement change only through the resource-update route and its checks.
    """
    from archihub.api.users.services import has_role
...
#
# They are parsed with a regular expression:
